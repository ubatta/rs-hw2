...
def maxfind(state):
    ...
    for k in range(len(state) - 1):
        for l in range(k + 1, len(state)):
            if (state[l] != state[k]) and (state[l] != state[k] + (l - k)) and (state[l] != state[k] - (l - k)):
                ftct = ftct + 1
    return ftct

cust_fit = mlrh.CustomFitness(maxfind)
...
neural_crv = []
varlist = []
restart_iteration = [1,100,200,300]
t1=time.time()
for r_i in restart_iteration:
    ...
    crvs = []
    nq_t = time.time()
    for i, j in enumerate(pynp.random.permutation(10) for hm in range(10)):
        test = mlrh.DiscreteOpt(length=10, fitness_fn=cust_fit, maximize=True, max_val=10)
...
pymap.title('Nqueens Problems - RHC')
pymap.xlabel('Restarts Iteration')
...
pymap.title('Nqueens Problems - SA')
...
pymap.title('Nqueens Problems - GA')
...
pymap.title('Nqueens Problems - MIMIC')
...
pymap.title('Nqueens Problems - MIMIC - Fitness')
pymap.xlabel('Percentage and Size of Sample')
...
pymap.title('Nqueens Problems - MIMIC - Fitness Curve')
pymap.xlabel('Percentage and Size of Sample')
pymap.ylabel("Fitness Curve")
pymap.legend(loc="best")
pymap.grid()
...
pymap.title('Nqueens Problems - MIMIC - Time')
pymap.xlabel('Percentage and Size of Sample')
pymap.ylabel("Time")
pymap.legend(loc="best")
pymap.grid()
